...
if [ $(jwhois $ns1|grep -o "No match for domain"|wc -l) -eq 1 ] ; then
    ...
    if [ $(whois $ns1|grep -o "No match for"|wc -l) -eq 1 ] ; then
        ...
        for ns in `whois $domain|grep -i 'name server'|head -n2|cut -d\: -f2` ; do
        whois $ns|head -n12|grep -A4 "Server Name:"|egrep -v "Whois|Referral"&&echo "" ;
        done
    fi
